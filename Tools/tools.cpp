...
namespace tools {
	...
	bool GetSingleWordIndex(std::string singleWord, int16_t* wordIndex) {
		...
		*wordIndex = -1;
		...
		bool isQuestionMark = ( 0 == strcmp(singleWord.c_str(), (const char*)"?") ) ;
		if (false == isQuestionMark)
		{
			for (int nIndex = 0; nIndex < 2048; nIndex++)
			{
				if (strcmp(singleWord.c_str(), (const char*)mnemonic_words[nIndex]) == 0)
				{
					*wordIndex = nIndex;
					found = true;
					break;
	...
	std::string GetMnemoString(int16_t* arr, uint8_t count, const uint8_t mnemonic_words[2048][9]) {
		...
		for (int i = 0; i < count - 1; i++) {
			words << mnemonic_words[arr[i]] <<" ";
		}
		words << mnemonic_words[ arr[count - 1] ];
		return words.str();
	}
	int stringToWordIndices(std::string str, int16_t *gen_words_indices) {
		...
		{
			bool found = GetSingleWordIndex(words[i].c_str(), &gen_words_indices[i]);
			if (!found) {
				std::cerr << "!!!ERROR PARSE STRING: \"" << str << "\"" << std::endl;
